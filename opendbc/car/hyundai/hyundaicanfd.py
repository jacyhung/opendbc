...
def create_ccnc(packer, CAN, openpilotLongitudinalControl, enabled, hud, leftBlinker, rightBlinker, msg_161, msg_162, msg_1b5,
  ...
  msg_161.update({
    ...
    "LANELINE_LEFT": (0 if not msg_1b5.get("Info_LftLnQualSta", 0) > 0 else 4 if hud.leftLaneDepart else 6 if leftBlinker or rightBlinker else 2),
    "LANELINE_RIGHT": (0 if not msg_1b5.get("Info_LftLnQualSta", 0) > 0 else 4 if hud.rightLaneDepart else 6 if leftBlinker or rightBlinker else 2),
